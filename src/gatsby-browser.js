/*const injectQuotebackScript = () => {
  function addJS(jsCode) {
    const s = document.createElement(`script`)
    window.quotebacks = 'lalala'

    s.type = `text/javascript`
    s.src = 'https://cdn.jsdelivr.net/gh/Blogger-Peer-Review/quotebacks@1/quoteback.js'
    document.getElementsByTagName(`head`)[0].appendChild(s)
  }
  addJS()
}*/

const activateQuoteback = () => {
  console.log('aaaa')
...
let injectedQuotebackScript = false
...
exports.onRouteUpdate = () => {
  window.quotebacks = embedClasses
  // If there's an embedded element, lazy-load the quoteback script (if it hasn't
  // already been loaded), and then run the quoteback load function.
  if (document.querySelector(embedClasses) !== null) {
    activateQuoteback()
    /*if (!injectedQuotebackScript) {
      activateQuoteback()
      injectedQuotebackScript = true
    }

    if (
      typeof quotebacks !== `undefined` &&
      window.quotebacks.widgets &&
      typeof window.quotebacks.widgets.load === `function`
    ) {
      window.quotebacks.widgets.load()
    }*/
  }
}
